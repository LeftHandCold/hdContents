...
void *
hd_malloc_array(hd_context *ctx, size_t count, size_t size)
{
    void *p;
    ...
    if (count > SIZE_MAX / size)
        hd_throw(ctx, HD_ERROR_MEMORY, "malloc of array (%zu x %zu bytes) failed (size_t overflow)", count, size);

    p = do_scavenging_malloc(ctx, count * size);
    if (!p)
        hd_throw(ctx, HD_ERROR_MEMORY, "malloc of array (%zu x %zu bytes) failed", count, size);
    return p;
}
