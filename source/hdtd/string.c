...
int
hd_strcasecmp(const char *a, const char *b)
{
    while (hd_tolower(*a) == hd_tolower(*b))
    {
        if (*a++ == 0)
            return 0;
        b++;
    }
    return hd_tolower(*a) - hd_tolower(*b);
}
...
void
hd_dirname(char *dir, const char *path, size_t n)
{
    size_t i;
    ...
    hd_strlcpy(dir, path, n);

    i = strlen(dir);
    for(; dir[i] == '/'; --i) if (!i) { hd_strlcpy(dir, "/", n); return; }
    for(; dir[i] != '/'; --i) if (!i) { hd_strlcpy(dir, ".", n); return; }
    for(; dir[i] == '/'; --i) if (!i) { hd_strlcpy(dir, "/", n); return; }
    dir[i+1] = 0;
}
...
void
hd_format_output_path(hd_context *ctx, char *path, size_t size, const char *fmt, int page)
{
    const char *s, *p;
    char num[40];
    int i, n;
    ...
    if (n + i + strlen(p) >= size)
        hd_throw(ctx, HD_ERROR_GENERIC, "path name buffer overflow");
    memcpy(path, fmt, n);
    while (i > 0)
        path[n++] = num[--i];
    hd_strlcpy(path + n, p, size - n);
}

#define SEP(x) ((x)=='/' || (x) == 0)

char *
hd_cleanname(char *name)
{
    char *p, *q, *dotdot;
    int rooted;
...
int
hd_runetochar(char *str, int rune)
{
...
hd_off_t hd_atoo(const char *s)
{
    if (s == NULL)
        return 0;
    return hd_atoo_imp(s);
}
