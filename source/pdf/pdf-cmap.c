...
pdf_drop_cmap_imp(hd_context *ctx, hd_storable *cmap_)
{
    ...
    hd_free(ctx, cmap->dict);
    hd_free(ctx, cmap->tree);
    hd_free(ctx, cmap);
}
...
int
pdf_cmap_wmode(hd_context *ctx, pdf_cmap *cmap)
{
    return cmap->wmode;
}

void
pdf_set_cmap_wmode(hd_context *ctx, pdf_cmap *cmap, int wmode)
{
    cmap->wmode = wmode;
}
 ...
 */
void
pdf_add_codespace(hd_context *ctx, pdf_cmap *cmap, unsigned int low, unsigned int high, int n)
{
    if (cmap->codespace_len + 1 == nelem(cmap->codespace))
    {
        hd_warn(ctx, "assert: too many code space ranges");
        return;
    }
 ...
 */
static void
add_range(hd_context *ctx, pdf_cmap *cmap, unsigned int low, unsigned int high, unsigned int out, int check_for_overlap, int many)
{
    int current;
    cmap_splay *tree;
    ...
    if (low > high)
    {
        hd_warn(ctx, "range limits out of range in cmap %s", cmap->cmap_name);
        return;
    }
    ...
    if (cmap->tlen == cmap->tcap)
    {
        ...
        tree = cmap->tree = hd_resize_array(ctx, cmap->tree, new_cap, sizeof *cmap->tree);
        cmap->tcap = new_cap;
    }
 ...
 */
void
pdf_map_range_to_range(hd_context *ctx, pdf_cmap *cmap, unsigned int low, unsigned int high, int out)
{
...
void
pdf_sort_cmap(hd_context *ctx, pdf_cmap *cmap)
{
    int counts[3];
    ...
    cmap->ranges = hd_malloc_array(ctx, counts[0], sizeof(*cmap->ranges));
    cmap->rcap = counts[0];
    cmap->xranges = hd_malloc_array(ctx, counts[1], sizeof(*cmap->xranges));
    cmap->xcap = counts[1];
    cmap->mranges = hd_malloc_array(ctx, counts[2], sizeof(*cmap->mranges));
    cmap->mcap = counts[2];
 ...
 */
static void
add_mrange(hd_context *ctx, pdf_cmap *cmap, unsigned int low, int *out, int len)
{
    int out_pos;
    ...
    if (cmap->dlen + len + 1 > cmap->dcap)
    {
        ...
        cmap->dict = hd_resize_array(ctx, cmap->dict, new_cap, sizeof *cmap->dict);
        cmap->dcap = new_cap;
    }
    out_pos = cmap->dlen;
 ...
 */
void
pdf_map_one_to_many(hd_context *ctx, pdf_cmap *cmap, unsigned int low, int *values, int len)
{
    if (len == 1)
    {
    ...
    {
        hd_warn(ctx, "ignoring one to many mapping in cmap %s", cmap->cmap_name);
        return;
    }
