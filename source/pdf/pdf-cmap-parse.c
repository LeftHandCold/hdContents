...
static void
pdf_parse_cmap_name(hd_context *ctx, pdf_cmap *cmap, hd_stream *file, pdf_lexbuf *buf)
{
    pdf_token tok;
    ...
    if (tok == PDF_TOK_NAME)
        hd_strlcpy(cmap->cmap_name, buf->scratch, sizeof(cmap->cmap_name));
    else
        hd_warn(ctx, "expected name after CMapName in cmap");
}

static void
pdf_parse_wmode(hd_context *ctx, pdf_cmap *cmap, hd_stream *file, pdf_lexbuf *buf)
{
    pdf_token tok;
    ...
    if (tok == PDF_TOK_INT)
        ...
        hd_warn(ctx, "expected integer after WMode in cmap");
}

static void
pdf_parse_codespace_range(hd_context *ctx, pdf_cmap *cmap, hd_stream *file, pdf_lexbuf *buf)
{
    pdf_token tok;
    int lo, hi;
    ...
    hd_throw(ctx, HD_ERROR_GENERIC, "expected string or endcodespacerange");
}

static void
pdf_parse_cid_range(hd_context *ctx, pdf_cmap *cmap, hd_stream *file, pdf_lexbuf *buf)
{
    pdf_token tok;
    int lo, hi, dst;
    ...
    while (1)
    {
        ...
        if (tok == PDF_TOK_KEYWORD && !strcmp(buf->scratch, "endcidrange"))
            ...
            hd_throw(ctx, HD_ERROR_GENERIC, "expected string or endcidrange");

        lo = pdf_code_from_string(buf->scratch, buf->len);

        tok = pdf_lex(ctx, file, buf);
        if (tok != PDF_TOK_STRING)
            hd_throw(ctx, HD_ERROR_GENERIC, "expected string");

        hi = pdf_code_from_string(buf->scratch, buf->len);

        tok = pdf_lex(ctx, file, buf);
        if (tok != PDF_TOK_INT)
            hd_throw(ctx, HD_ERROR_GENERIC, "expected integer");
...
static void
pdf_parse_cid_char(hd_context *ctx, pdf_cmap *cmap, hd_stream *file, pdf_lexbuf *buf)
{
    pdf_token tok;
    int src, dst;
    ...
    while (1)
    {
        ...
        if (tok == PDF_TOK_KEYWORD && !strcmp(buf->scratch, "endcidchar"))
            ...
            hd_throw(ctx, HD_ERROR_GENERIC, "expected string or endcidchar");
...
static void
pdf_parse_bf_range_array(hd_context *ctx, pdf_cmap *cmap, hd_stream *file, pdf_lexbuf *buf, int lo, int hi)
{
    pdf_token tok;
    int dst[256];
    int i;
    ...
    while (1)
    {
        ...
        if (tok == PDF_TOK_CLOSE_ARRAY)
            ...
            hd_throw(ctx, HD_ERROR_GENERIC, "expected string or ]");

        if (buf->len / 2)
        {
            int len = hd_mini(buf->len / 2, nelem(dst));
            for (i = 0; i < len; i++)
...
static void
pdf_parse_bf_range(hd_context *ctx, pdf_cmap *cmap, hd_stream *file, pdf_lexbuf *buf)
{
    pdf_token tok;
    int lo, hi, dst;
    ...
    while (1)
    {
        ...
        if (tok == PDF_TOK_KEYWORD && !strcmp(buf->scratch, "endbfrange"))
            ...
            hd_throw(ctx, HD_ERROR_GENERIC, "expected string or endbfrange");
        ...
        {
            hd_warn(ctx, "bf_range limits out of range in cmap %s", cmap->cmap_name);
            return;
        }
        ...
        if (tok == PDF_TOK_STRING)
        {
            ...
            else
            {
                ...
                if (buf->len / 2)
                {
                    int len = hd_mini(buf->len / 2, nelem(dststr));
                    for (i = 0; i < len; i++)
        ...
        {
            hd_throw(ctx, HD_ERROR_GENERIC, "expected string or array or endbfrange");
        }
    }
}

static void
pdf_parse_bf_char(hd_context *ctx, pdf_cmap *cmap, hd_stream *file, pdf_lexbuf *buf)
{
    pdf_token tok;
    int dst[256];
    int src;
    ...
    while (1)
    {
        ...
        if (tok == PDF_TOK_KEYWORD && !strcmp(buf->scratch, "endbfchar"))
            ...
            hd_throw(ctx, HD_ERROR_GENERIC, "expected string or endbfchar");
...
pdf_cmap *
pdf_load_cmap(hd_context *ctx, hd_stream *file)
{
    pdf_cmap *cmap;
    char key[64];
    pdf_lexbuf buf;
        ...
        while (1)
        {
            ...
            else if (tok == PDF_TOK_NAME)
            {
                if (!strcmp(buf.scratch, "CMapName"))
                    ...
                    hd_strlcpy(key, buf.scratch, sizeof key);
            }
            ...
            else if (tok == PDF_TOK_KEYWORD)
            {
                if (!strcmp(buf.scratch, "endcmap"))
                    ...
                    hd_strlcpy(cmap->usecmap_name, key, sizeof(cmap->usecmap_name));
