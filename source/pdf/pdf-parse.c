...
pdf_obj *
pdf_parse_indirect_obj(hd_context *ctx, pdf_document *doc,
                  hd_stream *file, pdf_lexbuf *buf,
                  int *onum, int *ogen, hd_off_t *ostmofs)
{
    pdf_obj *obj = NULL;
    int num = 0, gen = 0;
    hd_off_t stm_ofs;
    ...
    if (tok != PDF_TOK_INT)
    {
        hd_throw(ctx, HD_ERROR_SYNTAX, "expected object number");
    }
    num = buf->i;
    ...
    if (tok != PDF_TOK_INT)
    {
        hd_throw(ctx, HD_ERROR_SYNTAX, "expected generation number (%d ? obj)", num);
    }
    gen = buf->i;
    ...
    if (tok != PDF_TOK_OBJ)
    {
        hd_throw(ctx, HD_ERROR_SYNTAX, "expected 'obj' keyword (%d %d ?)", num, gen);
    }
    ...
    switch (tok)
    {
        ...
        case PDF_TOK_INT:
            ...
            hd_throw(ctx, HD_ERROR_SYNTAX, "expected 'R' keyword (%d %d R)", num, gen);
        ...
        default:
            hd_throw(ctx, HD_ERROR_SYNTAX, "syntax error in object (%d %d R)", num, gen);
    }
    ...
    hd_try(ctx)
    {
        ...
        if (tok == PDF_TOK_STREAM)
        {
            ...
            if (c == '\r')
            {
                ...
                if (c != '\n')
                    hd_warn(ctx, "line feed missing after stream begin marker (%d %d R)", num, gen);
                else
                    hd_read_byte(ctx, file);
            }
            stm_ofs = hd_tell(ctx, file);
        }
        ...
        {
            hd_warn(ctx, "expected 'endobj' or 'stream' keyword (%d %d R)", num, gen);
            stm_ofs = 0;
        }
    }
    hd_catch(ctx)
