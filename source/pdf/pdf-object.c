...
pdf_obj *
pdf_new_null(hd_context *ctx, pdf_document *doc)
{
    return PDF_OBJ_NULL;
}

pdf_obj *
pdf_new_bool(hd_context *ctx, pdf_document *doc, int b)
{
...
{
    pdf_obj_num *obj;
    obj = Memento_label(hd_malloc(ctx, sizeof(pdf_obj_num)), "pdf_obj(real)");
    obj->super.refs = 1;
...
pdf_obj *
pdf_new_name(hd_context *ctx, pdf_document *doc, const char *str)
{
    pdf_obj_name *obj;
    char **stdname;
...
pdf_obj *
pdf_new_indirect(hd_context *ctx, pdf_document *doc, int num, int gen)
{
    pdf_obj_ref *obj;
    obj = Memento_label(hd_malloc(ctx, sizeof(pdf_obj_ref)), "pdf_obj(indirect)");
    obj->super.refs = 1;
...
pdf_obj *
pdf_keep_obj(hd_context *ctx, pdf_obj *obj)
{
    if (obj >= PDF_OBJ__LIMIT)
    {
        (void)hd_keep_imp16(ctx, obj, &obj->refs);
    }
    return obj;
}
...
void pdf_set_int(hd_context *ctx, pdf_obj *obj, int64_t i)
{
    if (!OBJ_IS_INT(obj))
        return;
    NUM(obj)->u.i = i;
...
void pdf_set_str_len(hd_context *ctx, pdf_obj *obj, int newlen)
{
    RESOLVE(obj);
    if (!OBJ_IS_STRING(obj))
...
static void
pdf_array_grow(hd_context *ctx, pdf_obj_array *obj)
{
    int i;
    int new_cap = (obj->cap * 3) / 2;

    obj->items = hd_resize_array(ctx, obj->items, new_cap, sizeof(pdf_obj*));
    obj->cap = new_cap;
...
pdf_obj *
pdf_copy_array(hd_context *ctx, pdf_obj *obj)
{
    pdf_document *doc;
    pdf_obj *arr;
    int i;
...
int
pdf_array_len(hd_context *ctx, pdf_obj *obj)
{
    RESOLVE(obj);
    if (!OBJ_IS_ARRAY(obj))
        return 0;
...
pdf_obj *
pdf_array_get(hd_context *ctx, pdf_obj *obj, int i)
{
    RESOLVE(obj);
    if (!OBJ_IS_ARRAY(obj))
        return NULL;
...
static void prepare_object_for_alteration(hd_context *ctx, pdf_obj *obj, pdf_obj *val)
{
    ...
    */
    ...
    if (val)
    {
        ...
        if (doc && val_doc && val_doc != doc)
            hd_throw(ctx, HD_ERROR_GENERIC, "container and item belong to different documents");
    }
...
void
pdf_array_put(hd_context *ctx, pdf_obj *obj, int i, pdf_obj *item)
{
    RESOLVE(obj);
    if (!OBJ_IS_ARRAY(obj))
        hd_throw(ctx, HD_ERROR_GENERIC, "not an array (%s)", pdf_objkindstr(obj));
...
void
pdf_array_put_drop(hd_context *ctx, pdf_obj *obj, int i, pdf_obj *item)
{
    hd_try(ctx)
...
void
pdf_array_insert(hd_context *ctx, pdf_obj *obj, pdf_obj *item, int i)
{
    RESOLVE(obj);
    if (!OBJ_IS_ARRAY(obj))
        hd_throw(ctx, HD_ERROR_GENERIC, "not an array (%s)", pdf_objkindstr(obj));
...
void
pdf_array_insert_drop(hd_context *ctx, pdf_obj *obj, pdf_obj *item, int i)
{
    RESOLVE(obj);
    if (obj >= PDF_OBJ__LIMIT)
    {
...
int
pdf_array_contains(hd_context *ctx, pdf_obj *arr, pdf_obj *obj)
{
    int i, len;
...
int
pdf_array_find(hd_context *ctx, pdf_obj *arr, pdf_obj *obj)
{
    int i, len;
...
pdf_obj *
pdf_new_dict(hd_context *ctx, pdf_document *doc, int initialcap)
{
    pdf_obj_dict *obj;
    int i;

    obj = Memento_label(hd_malloc(ctx, sizeof(pdf_obj_dict)), "pdf_obj(dict)");
    obj->super.refs = 1;
    ...
    hd_try(ctx)
    {
        DICT(obj)->items = Memento_label(hd_malloc_array(ctx, DICT(obj)->cap, sizeof(struct keyval)), "pdf_obj(dict items)");
    }
    hd_catch(ctx)
    {
        hd_free(ctx, obj);
...
static void
pdf_dict_grow(hd_context *ctx, pdf_obj *obj)
{
    int i;
    int new_cap = (DICT(obj)->cap * 3) / 2;

    DICT(obj)->items = hd_resize_array(ctx, DICT(obj)->items, new_cap, sizeof(struct keyval));
...
pdf_obj *
pdf_copy_dict(hd_context *ctx, pdf_obj *obj)
{
    pdf_document *doc;
    pdf_obj *dict;
    int i, n;
...
int
pdf_dict_len(hd_context *ctx, pdf_obj *obj)
{
    RESOLVE(obj);
    if (!OBJ_IS_DICT(obj))
        return 0;
...
pdf_obj *
pdf_dict_get_key(hd_context *ctx, pdf_obj *obj, int i)
{
    RESOLVE(obj);
    if (!OBJ_IS_DICT(obj))
        return NULL;
...
pdf_obj *
pdf_dict_get_val(hd_context *ctx, pdf_obj *obj, int i)
{
    RESOLVE(obj);
    if (!OBJ_IS_DICT(obj))
        return NULL;
...
void
pdf_dict_put_val_null(hd_context *ctx, pdf_obj *obj, int idx)
{
    RESOLVE(obj);
    if (!OBJ_IS_DICT(obj))
        hd_throw(ctx, HD_ERROR_GENERIC, "not a dict (%s)", pdf_objkindstr(obj));
...
static int
pdf_dict_finds(hd_context *ctx, pdf_obj *obj, const char *key)
{
...
static int
pdf_dict_find(hd_context *ctx, pdf_obj *obj, pdf_obj *key)
{
    int len = DICT(obj)->len;
    int i;
    for (i = 0; i < len; i++)
...
pdf_obj *
pdf_dict_gets(hd_context *ctx, pdf_obj *obj, const char *key)
{
    int i;
...
pdf_obj *
pdf_dict_getp(hd_context *ctx, pdf_obj *obj, const char *keys)
{
    char buf[256];
    char *k, *e;
    ...
    if (strlen(keys)+1 > 256)
        hd_throw(ctx, HD_ERROR_GENERIC, "path too long");
...
pdf_obj *
pdf_dict_getl(hd_context *ctx, pdf_obj *obj, ...)
{
    va_list keys;
    pdf_obj *key;
...
pdf_obj *
pdf_dict_get(hd_context *ctx, pdf_obj *obj, pdf_obj *key)
{
    int i;
...
pdf_obj *
pdf_dict_getsa(hd_context *ctx, pdf_obj *obj, const char *key, const char *abbrev)
{
    pdf_obj *v;
...
pdf_obj *
pdf_dict_geta(hd_context *ctx, pdf_obj *obj, pdf_obj *key, pdf_obj *abbrev)
{
    pdf_obj *v;
...
static void
pdf_dict_get_put(hd_context *ctx, pdf_obj *obj, pdf_obj *key, pdf_obj *val, pdf_obj **old_val)
{
    int i;
    ...
    if (!OBJ_IS_DICT(obj))
        hd_throw(ctx, HD_ERROR_GENERIC, "not a dict (%s)", pdf_objkindstr(obj));
    if (!OBJ_IS_NAME(key))
        hd_throw(ctx, HD_ERROR_GENERIC, "key is not a name (%s)", pdf_objkindstr(obj));
...
void
pdf_dict_put(hd_context *ctx, pdf_obj *obj, pdf_obj *key, pdf_obj *val)
{
    pdf_dict_get_put(ctx, obj, key, val, NULL);
}

void
pdf_dict_put_drop(hd_context *ctx, pdf_obj *obj, pdf_obj *key, pdf_obj *val)
{
    hd_try(ctx)
    pdf_dict_get_put(ctx, obj, key, val, NULL);
    hd_always(ctx)
...
void
pdf_dict_get_put_drop(hd_context *ctx, pdf_obj *obj, pdf_obj *key, pdf_obj *val, pdf_obj **old_val)
{
    hd_try(ctx)
...
void
pdf_dict_puts(hd_context *ctx, pdf_obj *obj, const char *key, pdf_obj *val)
{
    pdf_document *doc;
    pdf_obj *keyobj;
...
void
pdf_dict_puts_drop(hd_context *ctx, pdf_obj *obj, const char *key, pdf_obj *val)
{
    pdf_document *doc;
    pdf_obj *keyobj;
...
void
pdf_dict_putp(hd_context *ctx, pdf_obj *obj, const char *keys, pdf_obj *val)
{
    pdf_document *doc;
    char buf[256];
    char *k, *e;
    ...
    if (!OBJ_IS_DICT(obj))
        hd_throw(ctx, HD_ERROR_GENERIC, "not a dict (%s)", pdf_objkindstr(obj));
    if (strlen(keys)+1 > 256)
        hd_throw(ctx, HD_ERROR_GENERIC, "buffer overflow in pdf_dict_putp");
...
void
pdf_dict_putp_drop(hd_context *ctx, pdf_obj *obj, const char *keys, pdf_obj *val)
{
    hd_try(ctx)
...
void
pdf_dict_putl_drop(hd_context *ctx, pdf_obj *obj, pdf_obj *val, ...)
{
    va_list keys;
    va_start(keys, val);
...
void
pdf_dict_dels(hd_context *ctx, pdf_obj *obj, const char *key)
{
    int i;
    ...
    if (!OBJ_IS_DICT(obj))
        hd_throw(ctx, HD_ERROR_GENERIC, "not a dict (%s)", pdf_objkindstr(obj));
    if (!key)
        hd_throw(ctx, HD_ERROR_GENERIC, "key is null");
...
pdf_obj *
pdf_deep_copy_obj(hd_context *ctx, pdf_obj *obj)
{
    if (obj < PDF_OBJ__LIMIT)
    {
...
int
pdf_obj_marked(hd_context *ctx, pdf_obj *obj)
{
    RESOLVE(obj);
    if (obj < PDF_OBJ__LIMIT)
        return 0;
...
int
pdf_mark_obj(hd_context *ctx, pdf_obj *obj)
{
    int marked;
    RESOLVE(obj);
    if (obj < PDF_OBJ__LIMIT)
...
void
pdf_unmark_obj(hd_context *ctx, pdf_obj *obj)
{
    RESOLVE(obj);
    if (obj < PDF_OBJ__LIMIT)
        return;
...
void
pdf_set_obj_memo(hd_context *ctx, pdf_obj *obj, int bit, int memo)
{
    if (obj < PDF_OBJ__LIMIT)
        return;
...
int
pdf_obj_memo(hd_context *ctx, pdf_obj *obj, int bit, int *memo)
{
    if (obj < PDF_OBJ__LIMIT)
        return 0;
    bit <<= 1;
...
int pdf_obj_is_dirty(hd_context *ctx, pdf_obj *obj)
{
    RESOLVE(obj);
    if (obj < PDF_OBJ__LIMIT)
        return 0;
...
void pdf_dirty_obj(hd_context *ctx, pdf_obj *obj)
{
    RESOLVE(obj);
    if (obj < PDF_OBJ__LIMIT)
        return;
...
void pdf_clean_obj(hd_context *ctx, pdf_obj *obj)
{
    if (obj < PDF_OBJ__LIMIT)
        return;
    obj->flags &= ~PDF_FLAGS_DIRTY;
...
static void
pdf_drop_array(hd_context *ctx, pdf_obj *obj)
{
    int i;
    ...
    hd_free(ctx, DICT(obj)->items);
    hd_free(ctx, obj);
}

static void
pdf_drop_dict(hd_context *ctx, pdf_obj *obj)
{
    int i;
